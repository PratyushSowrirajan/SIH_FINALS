...
df = pd.read_csv('qwerty_dataset_20251212_060650.csv')
...
for trial in [1, 2, 3, 4, 5]:
    ...
    # Check orientation for trial 3
    if trial == 3:
        avg_az = t["az"].mean()
        if avg_az < -0.5:
            print('  ✅ UPSIDE DOWN DETECTED (az is negative)')
        else:
            print('  ⚠️  Expected negative az for upside down')

print('\n' + '='*60)
print('Expected behavior:')
print('  Trials 1,2,5: Small variations (static)')
print('  Trial 3: az should be NEGATIVE (upside down)')
print('  Trial 4: Large gyro/accel variations (random shake)')
